...
### 1.7. Expand Tasks Pending Subtasks (v4.9)

> **Skill-Based Expansion**: As of v4.9, subtask expansion is handled by the `subtask-expansion` skill. This centralizes complexity heuristics and TDD template generation.

**Why Skill Extraction (v4.9 Change):**
- Reusable across different workflows (execute-tasks, PR review cycle)
- Configurable complexity heuristics in one location
- Supports `complexity_override` field in tasks.json
...
2. FOR EACH task with needs_subtask_expansion:
   ...
   // Invoke subtask-expansion skill
   const expansionResult = await Skill({
     ...
     args: JSON.stringify({
       task: {
         ...
         file_context: task.file_context || null,
         source: task.source || "spec"
       },
   ...
   IF expansionResult.status === "success":
     // Update tasks.json with generated subtasks
     jq --arg tid "${task.id}" --argjson result "${JSON.stringify(expansionResult)}" '
       .tasks |= map(
         if .id == $tid then
           del(.needs_subtask_expansion) |
           .subtasks = [$result.subtasks[].id] |
           .type = "parent"
         else . end
       ) |
     ...
     // Log expansion
     LOG: "Expanded task ${task.id}: ${expansionResult.subtasks.length} subtasks (${expansionResult.complexity_detected})"

   ELSE:
     WARN: "Subtask expansion failed for ${task.id}: ${expansionResult.error}"
     // Task remains with needs_subtask_expansion flag for retry

3. HANDLE legacy WAVE_TASK items (merged from deprecated Step 1.8):

   // Check for orphaned WAVE_TASK items in future_tasks
   const legacyItems = await Bash('bash "${CLAUDE_PROJECT_DIR}/.claude/scripts/task-operations.sh" list-wave-tasks [spec-name]');

   IF legacyItems.count > 0:
     WARN: "Found ${legacyItems.count} legacy WAVE_TASK items - auto-promoting"
     
     // Graduate legacy items to tasks with needs_subtask_expansion
     await Bash('bash "${CLAUDE_PROJECT_DIR}/.claude/scripts/task-operations.sh" graduate-all [spec-name]');
     
     // Re-run expansion for newly promoted tasks
     RECURSE: Step 1.7 for newly promoted tasks

4. TRACK expansions:
   expanded_tasks = [{ task_id, subtask_count, complexity, description }]
...
5. INCLUDE in output:
   "on_demand_expanded": {
     ...
     "skill_version": "1.0.0",
     "tasks": [
       { "task_id": "8", "subtasks": 4, "complexity": "MEDIUM", "description": "Add retry logic" }
     ],
     "legacy_migrated": legacyCount
   }
```

**Complexity Override Support (v4.9):**
```
Tasks can specify complexity_override in tasks.json:

{
  "id": "5",
  "description": "Simple fix that looks complex",
  "complexity_override": "LOW"  // Forces 3 subtasks despite keywords
}

The skill respects this override and logs:
  "complexity_source": "complexity_override"
```
