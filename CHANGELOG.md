...
- **Execute Spec Resume Bug**: Fixed issue where `/execute-spec` wouldn't resume orchestrator when restarting mid-execution
  - Root cause: Command file lacked explicit handling for "state exists, no flags" resume scenario
  - Fix 1: Added Step 5 (Check Existing State) to `execute-spec.md` with explicit resume flow
  - Fix 2: Added RESUME BEHAVIOR documentation to orchestrator's Main Orchestration Loop
  - Fix 3: Added "On Resume" note to AWAITING_REVIEW phase handler
  - Now explicitly spawns orchestrator whether starting fresh or resuming
