...
`
  ...
  if (result.status === "success") {
    ...
    if (TEAMS_ENABLED) {
      // v5.1.0: Teams-based review watching
      // Spawn review-watcher teammate instead of sleep-and-re-invoke
      INFORM: `Spawning review-watcher teammate for PR #${result.pr_number}...`

      const review_team = `review-${spec_name}-${wave}`;
      TeamCreate({ team_name: review_team, description: `Review watch for ${spec_name} wave ${wave}` });

      // Spawn review-watcher as teammate
      Task({
      ...
      // Wait for message from review-watcher (blocks until message arrives)
      // The review-watcher will send a message when review is found or on timeout
      // Message delivery is automatic — we just wait for it

      // When we receive the message:
      // - If review_received: re-invoke wave with AWAITING_REVIEW resume
      // - If review_timeout: return timeout status

      // Cleanup review team
      SendMessage({
        type: "shutdown_request",
        recipient: "watcher",
        content: "Review check complete"
      });
      TeamDelete();

      // Re-invoke wave-lifecycle-agent with resume_phase
      resume_phase = "AWAITING_REVIEW"
      continue WAVE_LOOP

    } else {
      // Legacy: Wait 2 minutes then re-invoke (v5.0.1 behavior)
      INFORM: `Waiting 2 minutes before re-checking review status...`
      bash `sleep 120`

      resume_phase = "AWAITING_REVIEW"
      // Don't increment wave — re-run same wave
      continue WAVE_LOOP
    }

  } else if (result.status === "timeout") {
    // Review polling timed out - need manual intervention (legacy)
    bash `${CLAUDE_PROJECT_DIR}/.claude/scripts/execute-spec-operations.sh transition ${spec_name} AWAITING_REVIEW`
...
// Awaiting review - will re-check after delay (v5.0.1)
{
...
// Timeout - review polling timed out (manual check needed, legacy)
{
  status: "timeout",
  wave: 2,
  pr_number: 125
...
| Review not ready (Teams) | Spawn review-watcher teammate, wait for message |
| Review not ready (Legacy) | Wait 2 min, re-invoke wave with resume_phase |
| Review timeout | Transition to AWAITING_REVIEW, return timeout |
| Review-watcher fails | Fall back to legacy sleep-and-re-invoke |
...
### v5.1.0 (2026-02-09)
- Added Teams-based review watching (AGENT_OS_TEAMS=true)
- Spawns review-watcher teammate instead of sleep-and-re-invoke
- Teams tools added to frontmatter (TeamCreate, TeamDelete, SendMessage)
- Dual-mode review wait: Teams (message notification) or Legacy (2-min sleep)
- Fallback to legacy mode if review-watcher fails

### v5.0.1 (2026-02-09)
- Handle `awaiting_review` status from wave-lifecycle-agent
- Re-invoke wave after 2-minute delay instead of blocking poll
- Preserves review_cycle count across re-invocations
